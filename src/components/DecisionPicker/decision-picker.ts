...
import { DEFAULT_DURATION, MIN_DURATION, Routes, WheelState } from '../../constants';
import { loadOptionsFromStorage } from '../../utils/local-storage.ts';
import { Wheel } from './wheel.ts';
import { loadSoundStateFromStorage, saveSoundStateToStorage } from '../../utils/local-storage.ts';
...
export class DecisionPicker implements Component {
    ...
    public render(): HTMLElement {
        // Check if there are enough valid options
    ...
    private handleWheelStateChange = (state: WheelState, pickedOption: ValidOption | null): void => {
    ...
    private updatePickedOptionDisplay(): void {
        ...
        switch (this.currentState) {
            ...
            case WheelState.PICKING: {
                message = this.currentPickedOption ? this.currentPickedOption.title : 'Вращение...';
                this.pickedOptionElement.classList.remove('highlighted');
                break;
            }
            case WheelState.PICKED: {
                message = this.currentPickedOption ? this.currentPickedOption.title : 'Не удалось выбрать опцию';
    ...
    private updateControlsState(): void {
        if (!this.backButton || !this.soundButton || !this.durationInput || !this.pickButton) return;
    ...
    private toggleSound = (): void => {
        ...
        // Update wheel with new sound setting
        if (this.wheel) {
            this.wheel.reset(this.validOptions, this.soundEnabled);
        }
    }
