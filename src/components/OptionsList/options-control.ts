...
import {
  hasEnoughValidOptions,
  isValidJsonFile,
  parseJSONFile,
} from '../../utils/validation';
import {
  createAddValidOptionsModal,
  createPasteListModal,
} from './modal-windows.ts';
...
export class OptionsControl implements Component {
  ...
  private static createLoadButton(): HTMLButtonElement {
    return createElement({
      ...
      listeners: {
        click: (): void => {
          const fileInput =
            document.querySelector<HTMLInputElement>('#load-list-input');
  ...
  private createLoadContainer(): HTMLElement {
    return createElement({
      ...
      children: [OptionsControl.createLoadButton(), this.createFileInput()],
    });
  }
