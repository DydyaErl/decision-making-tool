...
export class Wheel {
  ...
  private calculateWheelSections(): void {
    ...
    this.totalWeight = this.options.reduce(
  ...
  private startWheelAnimation(durationSeconds: number): void {
    ...
    this.wheelCanvas.style.transition = `transform ${durationSeconds}s cubic-bezier(0.15, 0.85, 0.5, 1)`;
    this.wheelCanvas.style.transform = `rotate(${this.targetAngle}deg)`;

    this.wheelCanvas.addEventListener(
      'transitionend',
  ...
  private updateCurrentOption(): void {
    ...
    if (
      option &&
      (!this.currentOption || this.currentOption.id !== option.id)
    ) {
  ...
  private handleTransitionEnd = (): void => {
    ...
    if (this.wheelCanvas) {
      this.wheelCanvas.removeEventListener(
        'transitionend',
        this.handleTransitionEnd
      );
    }
  ...
  private getCurrentRotationDegrees(): number {
    ...
    if (this.state === WheelState.PICKING) {
      ...
      if (elapsed < this.animationDuration) {
        ...
        const rotationMatch = this.wheelCanvas.style.transform.match(
          /rotate\(([\d.]+)deg\)/
        );
        let targetDegrees = 0;
  ...
  private findSectionAtAngle(pointerRadians: number): ValidOption | null {
    ...
    for (const section of this.sectionInfo) {
      if (
        normalizedPointer >= section.startAngle &&
        normalizedPointer < section.endAngle
      ) {
    ...
    if (lastSection) {
      ...
      if (
        (lastSection.startAngle <= normalizedPointer &&
          normalizedPointer < 2 * Math.PI) ||
