...
function createPasteListModalContent(
  dialog: HTMLDialogElement,
  onConfirm: (options: Option[]) => void
): HTMLElement {
  return createElement({
    ...
    children: [
      ...
      createElement({
        ...
        textContent:
          'Enter your options in CSV format (title, weight), one per line:',
      }),
      createElement({
        tagName: 'textarea',
    ...
    children: [
      ...
      createElement({
        ...
        listeners: {
          click: (): void => {
            const textarea =
              dialog.querySelector<HTMLTextAreaElement>('.paste-field');
            if (textarea) {
...
function createValidOptionsModalContent(
  dialog: HTMLDialogElement
): HTMLElement {
  return createElement({
    tagName: 'div',
    ...
    children: [
      ...
      createElement({
        ...
        className: 'modal-description',
        textContent:
          'Please add at least two valid options. A valid option must have a title and a weight greater than 0.',
      }),
      createElement({
        tagName: 'div',
        className: 'modal-actions',
